$(function(){	
	...
	var JUEGO={
		...
		//se este moviendo,es una forma de bloquear las
		//teclas del usuario mientras se realiza el tiro y obtenemos el
		//resultado (gol, fuera o atajada)
	};
	
	//array para almacenar todas las teclas presionadas por el usuario
	//mientras estes pulsando las flechas funciona pero una vez sueltas deja de funcionar
	...
	var objJuego=setInterval(function(){
		...
		if(!JUEGO.TIROENPROCESO){
			...
			if(TECLAS_PRESIONADAS[TECLAS.DISPARO]){
				//Tiroenproceso= cuando es true no funcina ninguna tecla hasta que se complete el disparo
				JUEGO.TIROENPROCESO=true;
				$('#divBalon').stop(true,true).animate({
					//posicion de la linea de gol
						bottom: 435
				},{
					duration: 300,
					step:function(){
						...
						if(blnAtajado){
							...
							//sacamos un poco el portero de la cancha para a la hora de parar
							$('#divPortero').animate({
								bottom: -20 
							},function(){
							...
							$('#divBalon').stop(true,true).animate({
								...
								//desbloqueamos el juego para que el usuario pueda mover otra vez
							...
							//ocultamos el balon cuando metemos o cuando va fuera
							$('#divBalon').stop(true,true).fadeOut('fast',function(){
								//colocamos el balon en la posicion del ultimo tiro
								$(this).css('bottom',55);
								//mostramos el balon
								$(this).stop(true,true).fadeIn('fast',function(){
									//desbloqueamos para que el usuario siga jugando
									JUEGO.TIROENPROCESO=false;
								});
							});
						}
	...
	$.fntMoverPortero=function(){
		...
		$('#divPortero').stop(true,true).animate({
			...
			$(this).stop(true,true).animate({
				...
				//movemos el portero 100px a la derecha
				$(this).stop(true,true).animate({
					left: 100
				},iVelocidad,function(){
					//mover el portero 170px a la izquierda(lo devuelve al centro)
					$(this).stop(true,true).animate({
						left: 170
					},iVelocidad,function(){
